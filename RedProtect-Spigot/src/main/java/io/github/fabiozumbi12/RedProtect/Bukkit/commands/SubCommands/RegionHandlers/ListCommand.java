...
public class ListCommand implements SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof ConsoleCommandSender && args.length > 0) {
            //rp list [player]
            if (args.length == 1) {
                CommandHandlers.getRegionforList(sender, args[0], 1);
                return true;
            }
            //rp list [player] [page]
            if (args.length == 2) {
                try {
                    ...
                    CommandHandlers.getRegionforList(sender, args[0], Page);
                    return true;
            ...
            //rp list
            if (args.length == 0) {
                CommandHandlers.handleList(player, ((Player) sender).getUniqueId().toString(), 1);
                return true;
            }
            //rp list [player|page]
            if (args.length == 1) {
                try {
                    ...
                    CommandHandlers.getRegionforList(sender, ((Player) sender).getUniqueId().toString(), Page);
                    return true;
                } catch (NumberFormatException e) {
                    CommandHandlers.handleList(player, RedProtect.get().getUtil().PlayerToUUID(args[0]), 1);
                    return true;
                }
            }
            //rp list [player] [page]
            if (args.length == 2) {
                try {
                    int Page = Integer.parseInt(args[1]);
                    CommandHandlers.handleList(player, RedProtect.get().getUtil().PlayerToUUID(args[0]), Page);
                    return true;
                } catch (NumberFormatException e) {
