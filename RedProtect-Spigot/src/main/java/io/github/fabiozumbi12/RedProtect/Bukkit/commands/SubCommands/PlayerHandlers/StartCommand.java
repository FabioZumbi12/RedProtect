    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            RedProtect.get().getLanguageManager().sendMessage(player, RedProtect.get().getLanguageManager().get("cmdmanager.region.start.confirm")
                    .replace("{cmd}", label)
                    .replace("{confirm}", CommandHandlers.getCmd("confirm")));

            Bukkit.getScheduler().runTaskLater(RedProtect.get(), () -> RedProtect.get().confirmStart.remove(player.getName()), 600);
            return true;
        }

        if (args.length == 1 && CommandHandlers.checkCmd(args[0], "confirm")) {
            if (!RedProtect.get().confirmStart.contains(player.getName())) {
                player.sendMessage(RedProtect.get().getLanguageManager().get("cmdmanager.region.noconfirm").replace("{cmd}", CommandHandlers.getCmd("start")));
                return true;
            }
