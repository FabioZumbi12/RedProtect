...
public class Region extends CoreRegion {
    ...
    public String info() {
        ...
        if (RedProtect.get().hooks.checkDyn() && RedProtect.get().getConfigManager().configRoot().hooks.dynmap.enable) {
            dynmapInfo = RedProtect.get().getLanguageManager().get("region.map-info")
                    .replace("{hide}", this.getFlagBool("show-map") ? RedProtect.get().getLanguageManager().get("region.map-showing") : RedProtect.get().getLanguageManager().get("region.map-hiding"))
                    .replace("{set}", this.getDynmapSet())
                    + "\n";
        }
    ...
    public boolean canMining(Block b) {
        return flagExists("minefarm") && (b.getType().toString().contains("_ORE") || b.getType().equals(Material.STONE) || b.getType().toString().contains("GRASS") || b.getType().equals(Material.DIRT)) && getFlagBool("minefarm");
    }
