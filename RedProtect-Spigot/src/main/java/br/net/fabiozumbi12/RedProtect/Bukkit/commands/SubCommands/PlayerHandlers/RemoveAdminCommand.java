...
public class RemoveAdminCommand implements SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 3 && (sender instanceof ConsoleCommandSender || RedProtect.get().getPermissionHandler().hasPerm(sender, "redprotect.command.admin.removeadmin"))) {
            ...
            CommandHandlers.handleRemoveAdmin(sender, args[0], r);
            return true;
        } else if (sender instanceof Player player) {

            if (args.length == 1) {
                CommandHandlers.handleRemoveAdmin(player, args[0], null);
                return true;
            }
        }

        RedProtect.get().getLanguageManager().sendCommandHelp(sender, "removeadmin", true);
        return true;
    }
        ...
        if (sender instanceof Player) {
            ...
            if (r != null && args.length == 1) {
                if (args[0].isEmpty())
                    ...
                    return r.getAdmins().stream().map(PlayerRegion::getPlayerName).filter(playerName -> playerName.toLowerCase().startsWith(args[0].toLowerCase())).collect(Collectors.toList());
            }
        }
