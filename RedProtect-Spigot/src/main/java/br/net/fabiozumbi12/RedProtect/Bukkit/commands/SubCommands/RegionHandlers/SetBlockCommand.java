...
public class SetBlockCommand implements SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) {
            ...
            RedProtect.get().getBlockManager().setBlock(blocks, player);
            RedProtect.get().getLanguageManager().sendMessage(sender, "cmdmanager.region.totalblocks", new Replacer[]{
                    new Replacer("blocks", String.valueOf(blocks)),
                    new Replacer("player", player.getName())
            });
            return true;
        }

        RedProtect.get().getLanguageManager().sendCommandHelp(sender, "setblock", true);
        return true;
    }
