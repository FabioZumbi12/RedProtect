...
public class TeleportCommand implements SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player && args.length == 1) {
            CommandHandlers.handletp(sender, args[0], ((Player) sender).getWorld().getName(), null);
            return true;
        }

        if (args.length == 2) {
            CommandHandlers.handletp(sender, args[0], args[1], null);
            return true;
        }
        ...
        if (args.length == 3) {
            ...
            if (play != null) {
                CommandHandlers.handletp(sender, args[0], args[1], play);
            } else {
                RedProtect.get().getLanguageManager().sendMessage(sender, RedProtect.get().getLanguageManager().get("cmdmanager.noplayer.thisname").replace("{player}", args[2]));
            }
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 3)
            if (args[2].isEmpty())
                ...
                tab.addAll(Bukkit.getOnlinePlayers().stream().map(Player::getName).filter(name -> name.startsWith(args[2])).toList());
        return tab;
    }
}