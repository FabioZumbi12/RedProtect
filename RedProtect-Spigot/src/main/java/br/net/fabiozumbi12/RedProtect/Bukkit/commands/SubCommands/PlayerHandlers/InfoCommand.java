...
public class InfoCommand implements SubCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 2 && (sender instanceof ConsoleCommandSender || RedProtect.get().getPermissionHandler().hasPerm(sender, "redprotect.command.admin.info"))) {
            ...
            if (args.length == 0) {
                CommandHandlers.handleInfoTop(player);
                return true;
            }
            if (args.length == 1) {
                CommandHandlers.handleInfo(player, args[0], "");
                return true;
            }
            if (args.length == 2) {
                CommandHandlers.handleInfo(player, args[0], args[1]);
                return true;
            }
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            if (args[1].isEmpty())
                ...
                tab.addAll(Bukkit.getWorlds().stream().filter(e -> e.getName().startsWith(args[1])).map(World::getName).toList());
        }
        return tab;
    }
}