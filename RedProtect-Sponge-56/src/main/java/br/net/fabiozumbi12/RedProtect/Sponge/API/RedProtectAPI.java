...
public class RedProtectAPI {
    ...
    public Set<Region> getPlayerRegions(Player player) {
        return (Set) RedProtect.get().rm.getRegions(Sponge.getServer().getOnlineMode() ? player.getName() : player.getUniqueId().toString(), player.getWorld());
    }
    ...
    public Region getHighPriorityRegion(World world, int x, int y, int z) {
        return (Region) RedProtect.get().rm.getTopRegion(world, x, y, z, this.getClass().getName());
    }
