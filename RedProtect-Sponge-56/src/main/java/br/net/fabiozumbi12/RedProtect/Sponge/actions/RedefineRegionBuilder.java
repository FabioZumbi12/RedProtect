...
public class RedefineRegionBuilder extends RegionBuilder {
    ...
    public RedefineRegionBuilder(Player p, Region old, Location<World> loc1, Location<World> loc2) {
        ...
        if (loc1.getExtent().equals(loc2.getExtent()) && new Region(null, loc1, loc2, null).getArea() > RedProtect.get().config.configRoot().region_settings.max_scan && !RedProtect.get().ph.hasPerm(p, "redprotect.bypass.define-max-distance")) {
        ...
        int claimused = RedProtect.get().rm.getPlayerRegions(p.getName(), w.getName());
