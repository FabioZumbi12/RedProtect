...
public class CommandHandlers {
    ...
    public static void handleAddLeader(CommandSource src, String sVictim, Region r) {
        if (src instanceof Player) {
            Player p = (Player) src;
            r = RedProtect.get().getRegionManager().getTopRegion(p.getLocation(), CommandHandlers.class.getName());
            if (r == null) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermLeader(src, "addleader", r)) {
            ...
            if (!src.hasPermission("redprotect.command.admin.addleader")) {
                ...
                int claimused = RedProtect.get().getRegionManager().getPlayerRegions(pVictim.getName(), pVictim.getWorld().getName());
                ...
                Sponge.getScheduler().createSyncExecutor(RedProtect.get().container).schedule(() -> {
                    if (RedProtect.get().alWait.containsKey(pVictimf)) {
                        ...
                        if (src instanceof Player && ((Player) src).isOnline()) {
                            RedProtect.get().getLanguageManager().sendMessage(src, RedProtect.get().getLanguageManager().get("cmdmanager.region.leader.requestexpired").replace("{player}", pVictimf.getName()));
                        }
                    }
                }, RedProtect.get().getConfigManager().configRoot().region_settings.leadership_request_time, TimeUnit.SECONDS);
            } else {
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.leader.already"));
            }
    ...
    public static void handleRemoveLeader(CommandSource src, String sVictim, Region r) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermLeader(src, "removeleader", r)) {
            ...
            if (r.isLeader(sVictim)) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.leader.notleader"));
            }
        } else if (src instanceof Player) {
            RedProtect.get().getLanguageManager().sendMessage(src, "no.permission");
        }
    ...
    public static void handleAddAdmin(CommandSource src, String sVictim, Region r) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(src, "addadmin", r)) {
            ...
            if (!r.isAdmin(sVictim)) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.admin.already"));
            }
        } else if (src instanceof Player) {
            RedProtect.get().getLanguageManager().sendMessage(src, "no.permission");
        }
    ...
    public static void handleRemoveAdmin(CommandSource src, String sVictim, Region r) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(src, "removeadmin", r)) {
            ...
            if (r.isAdmin(sVictim)) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.admin.notadmin"));
            }
        } else if (src instanceof Player) {
            RedProtect.get().getLanguageManager().sendMessage(src, "no.permission");
        }
    ...
    public static void handleAddMember(CommandSource src, String sVictim, Region r) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(src, "addmember", r)) {
            ...
            if (r.isAdmin(sVictim)) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.member.already"));
            }
        } else if (src instanceof Player) {
            RedProtect.get().getLanguageManager().sendMessage(src, "no.permission");
        }
    ...
    public static void handleRemoveMember(CommandSource src, String sVictim, Region r) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(src, "removemember", r)) {
            ...
            if ((r.isMember(sVictim) || r.isAdmin(sVictim)) && !r.isLeader(sVictim)) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(src, "&c" + sVictim + " " + RedProtect.get().getLanguageManager().get("cmdmanager.region.member.notmember"));
            }
        } else if (src instanceof Player) {
            RedProtect.get().getLanguageManager().sendMessage(src, "no.permission");
        }
    ...
    public static void handleInfoTop(Player p) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(p, "info", r)) {
            ...
            if (groupr.size() > 1) {
                p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.moreregions")));
                for (Region regs : groupr.values()) {
                    if (regs != r) {
                        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("region.name") + " " + regs.getName() + " " + RedProtect.get().getLanguageManager().get("region.priority") + " " + regs.getPrior()));
                    }
                }
            }
        } else {
    ...
    public static void handletp(CommandSource source, String rname, World world, Player play) {
        Region region = RedProtect.get().getRegionManager().getRegion(rname, world.getName());
        if (region == null) {
            RedProtect.get().getLanguageManager().sendMessage(source, RedProtect.get().getLanguageManager().get("cmdmanager.region.doesntexist") + ": " + rname);
            return;
        }

        if (play == null) {
            if (source instanceof Player && !RedProtect.get().getPermissionHandler().hasRegionPermMember((Player) source, "teleport", region)) {
                RedProtect.get().getLanguageManager().sendMessage(source, "no.permission");
                return;
            }
        } else {
            if (!RedProtect.get().getPermissionHandler().hasPerm(source, "redprotect.command.admin.teleport")) {
                RedProtect.get().getLanguageManager().sendMessage(source, "no.permission");
                return;
            }
        }
        ...
        if (loc != null) {
            if (play != null) {
                ...
                RedProtect.get().getLanguageManager().sendMessage(source, RedProtect.get().getLanguageManager().get("cmdmanager.region.tpother") + " " + rname);
    ...
    public static void handleWelcome(Player p, String wMessage) {
        ...
        if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(p, "welcome", r)) {
            if (r != null) {
                switch (wMessage) {
                    case "":
                        RedProtect.get().getLanguageManager().sendMessage(p, "cmdmanager.region.welcomeoff");
                        break;
                    case "hide ":
                        RedProtect.get().getLanguageManager().sendMessage(p, "cmdmanager.region.welcomehide");
                        break;
                    default:
                        RedProtect.get().getLanguageManager().sendMessage(p, RedProtect.get().getLanguageManager().get("cmdmanager.region.welcomeset") + " " + wMessage);
                        break;
                }
    ...
    public static void getRegionforList(CommandSource p, String uuid, int nPage) {
        Sponge.getScheduler().createAsyncExecutor(RedProtect.get()).execute(() -> {
            ...
            if (pname == null || length == 0) {
                RedProtect.get().getLanguageManager().sendMessage(p, "cmdmanager.player.noregions");
            } else {
                p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "-------------------------------------------------"));
                RedProtect.get().getLanguageManager().sendMessage(p, RedProtect.get().getLanguageManager().get("cmdmanager.region.created.list") + " " + pname);
                ...
                for (World w : Sponge.getServer().getWorlds()) {
                    ...
                    String colorChar = RedProtect.get().getConfigManager().configRoot().region_settings.world_colors.get(w.getName());
                    ...
                    if (wregions.size() > 0) {
                        ...
                        for (int i = min; i <= max; i++) {
                            ...
                            if (RedProtect.get().getPermissionHandler().hasRegionPermAdmin(p, "teleport", null)) {
                                if (first) {
                                    first = false;
                                    worldregions.append(Text.builder()
                                            .append(RedProtect.get().getUtil().toText("&8" + r.getName() + area))
                                            .onHover(TextActions.showText(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.list.hover").replace("{region}", r.getName()))))
                                            .onClick(TextActions.runCommand("/rp " + getCmd("teleport") + " " + r.getName() + " " + r.getWorld())).build());
                                } else {
                                    worldregions.append(Text.builder()
                                            .append(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + ", &8" + r.getName() + area))
                                            .onHover(TextActions.showText(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.list.hover").replace("{region}", r.getName()))))
                                            .onClick(TextActions.runCommand("/rp " + getCmd("teleport") + " " + r.getName() + " " + r.getWorld())).build());
                                }
                            } else {
                                if (first) {
                                    ...
                                    worldregions.append(Text.builder()
                                            .append(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + ", &8" + r.getName() + area)).build());
                                }
                            }
                            lastLocal = count;
                        }
                        ...
                        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + RedProtect.get().getLanguageManager().get("region.world").replace(":", "") + " " + colorChar + w.getName() + "[" + (min + 1) + "-" + (max + 1) + "/" + wregions.size() + "]&r: "));
                        p.sendMessages(worldregions.append(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + ".")).build());
                    }
                }
                p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "---------------- " + last + "/" + total + " -----------------"));
                if (last < total) {
                    p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.region.listpage.more").replace("{player}", pname + " " + (Page + 1))));
                } else {
                    if (Page != 1) {
                        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.region.listpage.nomore")));
                    }
                }
            }
        });
    ...
    private static void sendFlagHelp(Player p) {
        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "-------------[redprotect Flags]------------"));
        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.region.flag.list") + " " + RedProtect.get().getConfigManager().getDefFlags()));
        p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "------------------------------------"));
        ...
        if (sb.length() > 1) {
            p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.region.flag.admlist") + " [" + sb.toString().substring(0, sb.length() - 2) + "]"));
            p.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "------------------------------------"));
        }
    }
    ...
    public static void HandleHelpPage(CommandSource sender, int page) {
        sender.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("_redprotect.prefix") + " " + RedProtect.get().getLanguageManager().get("cmdmanager.available.cmds")));
        sender.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "------------------------------------"));
        sender.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.helpheader.alias")));
            ...
            for (String key : RedProtect.get().getLanguageManager().getHelpStrings()) {
                if (RedProtect.get().getPermissionHandler().hasCommandPerm(player, key) || ((key.equals("pos1") || key.equals("pos2")) && RedProtect.get().getPermissionHandler().hasCommandPerm(player, "redefine"))) {
                    ...
                    if (i > (page * 10) - 10 && i <= page * 10) {
                        player.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.help." + key).replace("{cmd}", getCmd(key)).replace("{alias}", getCmdAlias(key))));
                    }
                    if (i > page * 10) {
                        sender.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("general.color") + "------------------------------------"));
                        player.sendMessage(RedProtect.get().getUtil().toText(RedProtect.get().getLanguageManager().get("cmdmanager.page").replace("{page}", "" + (page + 1))));
                        break;
                    }
                }
            }
